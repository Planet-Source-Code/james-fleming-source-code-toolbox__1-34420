...
Begin VB.Form frmSearch 
   ...
   Begin TabDlg.SSTab sstSearch 
      ...
      TabHeight       =   520
      TabCaption(0)   =   "Files"
      TabPicture(0)   =   "frmSearch.frx":0008
      ...
      TabPicture(1)   =   "frmSearch.frx":0024
...
' Comment:  It is based loosely on source code found at Planet-source-code.
...
'*******************************************************
    ...
    Call ComboboxLoad(cmbTipType, "SELECT tblTipType.intTypeID, tblTipType.strTipType, tblTipType.strToolTip FROM tblTipType ORDER BY tblTipType.strTipType;") ' loads the tips combo box
...
Dim found As Integer
    Select Case Index
        ...
        Case 1  ' search for functions/subroutines
            ...
            If found = 0 Then
                Call StatusMsgDisplay("No " & UCase(Right(cmbExt, Len(frmSearch.cmbExt) - 2)) & " Procedures Found", 2)
            ElseIf found = 1 Then
                Call StatusMsgDisplay("1 " & UCase(Right(cmbExt, Len(frmSearch.cmbExt) - 2)) & " Procedure Found", 2)
            Else
                Call StatusMsgDisplay(found & " " & UCase(Right(cmbExt, Len(frmSearch.cmbExt) - 2)) & " Procedures Found", 2)
            End If                              ' end if
        Case Else
...
'Private Sub dlbDrive_LostFocus()
'   txtSearch.Text = dlbDrive.Drive & Right(txtSearch.Text, Len(txtSearch.Text) - 2)
'End Sub
...
If fLoad = True Then Exit Sub
    Select Case comboIndex
        Case 0  ' Access
            cmbExt.AddItem "*.mdb", 0
            cmbExt.Text = "*.mdb" ' set the default value
            
        Case 1  ' ASP
        
        Case 2  ' C
            cmbExt.AddItem "*.c", 0
            cmbExt.AddItem "*.cpp", 1
            cmbExt.AddItem "*.obj", 2
            cmbExt.Text = "*.c" ' set the default value
            
        Case 3  ' HTML
            cmbExt.AddItem "*.html", 0
            cmbExt.AddItem "*.htm", 1
            cmbExt.Text = "*.htm"
        ...
        Case 6  ' Office
            cmbExt.AddItem "*.doc", 0
            cmbExt.AddItem "*.xls", 1
            cmbExt.AddItem "*.mdb", 2
            cmbExt.Text = "*.doc"
        Case 7  ' PERL
        ...
        Case 10 ' VB
            cmbExt.AddItem "*.bas", 0
            cmbExt.AddItem "*.frm", 1
            cmbExt.AddItem "*.cls", 2
            cmbExt.AddItem "*.ctl", 3
            cmbExt.AddItem "*.pag", 4
            cmbExt.Text = "*.bas" ' set the default value
...
'    End
'End Sub
' COmmented out Feb 5 2000
'Private Sub mnuView_Click()
'On Error GoTo myErrHandler
'    If msgSearch(1).Col = 1 Then
'      TypeView = 1
'    Else
'      TypeView = 0
'    End If
'    StringToFind = msgSearch(1).Text
'    msgSearch(1).Col = 1
'    FileFSToOpen = msgSearch(1).Text
